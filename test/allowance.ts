...
export default () => {
  it("Initial allowance from Bob to Alice must be equal to zero", async function () {
  ...
  it("Allowance from Bob to Alice after approve must be equal to 40", async function () {
  ...
  it("Allowance from Bob to Alice after increaseAllowance must be equal to 50", async function () {
    await this.instance
      .connect(this.bob)
  ...
  it("Allowance from Bob to Alice after increaseAllowance and decreaseAllowance must be equal to 100", async function () {
    await this.instance
      .connect(this.bob)
  ...
  it("Sender can't decrease allowance below zero", async function () {
    await this.instance
      .connect(this.bob)
      .increaseAllowance(this.alice.address, 200);
  ...
  it("it is not possible to approve to zero address", async function () {
    await expect(
      this.instance.connect(this.bob).approve(this.zeroAddress, 40)
    ).to.be.revertedWith("Approve to the zero address");
  });
  it("it is not possible to approve from zero address", async function () {
    await expect(
